...
import { MessageSquare, Store, User, Clock, Search, RefreshCw } from 'lucide-react';
  ...
  const fetchConversations = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        if (initialConvId) {
          const matched = res.conversations.find((c) => c._id === initialConvId);
      ...
      <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm flex items-center justify-between">
        <div>
          <div className="flex items-center gap-2 text-brand-600 font-bold text-xs uppercase tracking-wider mb-1">
            ...
            <span>Direct Messaging Layer</span>
          </div>
          <h1 className="text-2xl font-black text-slate-900">
            Real-Time Messages & Shopkeeper Chat
          </h1>
          <p className="text-xs text-slate-500 mt-1">
            Context-protected live messaging with verified shop owners for inquiries, pricing, and pickup coordination.
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-3xl border border-slate-200 p-12 text-center space-y-3">
          ...
          <p className="text-xs text-slate-500 max-w-sm mx-auto">
            Start a chat by clicking "Chat" on any product card or broadcast comparison offer.
          </p>
        </div>
      ) : (
            ...
            <div className="space-y-2">
              {conversations.map((conv) => {
                ...
                return (
                  ...
                  >
                    <div className="w-10 h-10 rounded-xl bg-slate-900 text-brand-400 flex items-center justify-center font-bold flex-shrink-0 shadow-sm">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-500 truncate mt-0.5">
                        {conv.lastMessage?.text || 'Chat started'}
                      </p>
                    </div>
                  </div>
                );
